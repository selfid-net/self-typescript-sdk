...
export default class Crypto {
  ...
  constructor(client: IdentityService, device: string, storageFolder: string, storageKey: string) {
    ...
    this.accountPickleFile = `${storageFolder}/account.pickle`
  }
  ...
  public static async build(
    ...
    if (fs.existsSync(cc.accountPickleFile)) {
      // 1a) if alice's account file exists load the pickle from the file
      cc.account = crypto.unpickle_account(cc.accountPickleFile, cc.storageKey)
    } else {
      ...
      let res = await cc.client.postRaw(
        `${cc.client.url}/v1/identities/${cc.client.jwt.appID}/devices/1/pre_keys`,
        keys
      )
      if (res != 200) {
      ...
      fs.writeFileSync(cc.accountPickleFile, pickle, { mode: 0o600 })
    }
  ...
  public async encrypt(
    ...
    let session_file_name = `${this.storageFolder}/${recipient}:${recipientDevice}-session.pickle`
  ...
  public decrypt(message: string, sender: string, sender_device: string): Promise<string> {
    let session_file_name = `${this.storageFolder}/${sender}:${sender_device}-session.pickle`
    let session_with_bob
    ...
    if (fs.existsSync(session_file_name)) {
      ...
      let one_time_message =
        group_message_json['recipients'][`${this.client.jwt.appID}:${this.client.jwt.deviceID}`][
          'ciphertext'
        ]
      console.log('---> 1')
      console.log(one_time_message)
      ...
      fs.writeFileSync(this.accountPickleFile, account_pickle, { mode: 0o600 })
    }
