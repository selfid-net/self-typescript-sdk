...
export default class IdentityService {
    ...
    async devices(selfid: string): Promise<string[]> {
        ...
        try {
            ...
            const response = await axios.get(`https://api.review.selfid.net/v1/identities/${selfid}/devices`, options);
    ...
    async get(selfid: string): Promise<Identity> {
        ...
        try {
            ...
            const response = await axios.get(`https://api.review.selfid.net/v1/identities/${selfid}`, options);
            if(response.status === 200) {
